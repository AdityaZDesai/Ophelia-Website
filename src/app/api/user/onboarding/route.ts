...
export async function POST(request: Request) {
  try {
    ...
    if (!communicationChannel || !["imessage", "web", "whatsapp", "discord", "telegram"].includes(communicationChannel)) {
      return NextResponse.json(
    ...
    await pool.query(updateQuery, [
      communicationChannel,
      (communicationChannel === "imessage" || communicationChannel === "whatsapp") ? normalizedPhone : null,
      selectedPersonality,
      selectedPhoto,
      selectedAudio,
      session.user.id,
